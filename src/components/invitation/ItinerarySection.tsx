...
const EventCard = ({
  ...
  <AnimatedSection
    delay={delay}
    className="bg-card rounded-lg p-6 md:p-8 text-center max-w-sm w-full border border-border"
  >
    <Icon className="w-8 md:w-10 h-8 md:h-10 text-primary mx-auto mb-4" />
    <h3 className="text-lg md:text-xl font-serif font-semibold text-foreground mb-4">{info.nombre}</h3>
    ...
    <div className="flex items-start justify-center gap-2 text-muted-foreground mb-6">
      ...
      <span className="font-sans text-sm px-2">{info.direccion}</span>
    </div>
    <a
      href={info.maps_url}
      target="_blank"
      rel="noopener noreferrer"
      className="inline-flex items-center gap-2 text-xs md:text-sm font-sans text-primary hover:text-primary/80 transition-colors"
    >
...
const ItinerarySection = ({ ceremonia, recepcion }: Props) => (
  <>
    ...
    <AnimatedSection className="py-8 md:py-12 px-4 md:px-6 text-center">
      <h2 className="text-2xl md:text-4xl font-serif text-foreground mb-8 md:mb-12">Itinerario</h2>
