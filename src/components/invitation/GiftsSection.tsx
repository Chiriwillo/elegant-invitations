...
const GiftsSection = ({ regalos, transferencia }: Props) => (
  <>
    ...
    <AnimatedSection className="py-12 px-6 text-center">
      ...
      <h2 className="text-3xl md:text-4xl font-serif text-foreground mb-4">Mesa de regalos</h2>
      ...
      <div className="flex flex-col md:flex-row items-stretch justify-center gap-6 max-w-2xl mx-auto mb-10">
        {regalos.map((r, i) => (
          <AnimatedSection
            key={i}
            delay={i * 0.15}
          ...
          >
            <h3 className="font-serif font-semibold text-foreground mb-2">{r.titulo}</h3>
            <p className="text-sm font-sans text-muted-foreground mb-4">{r.descripcion}</p>
            <a
              href={r.url}
              target="_blank"
              rel="noopener noreferrer"
              className="inline-flex items-center gap-1.5 text-sm font-sans text-primary hover:text-primary/80 transition-colors"
            >
      ...
      {transferencia && (
        <AnimatedSection delay={0.3} className="bg-card border border-border rounded-lg p-6 max-w-sm mx-auto text-left">
